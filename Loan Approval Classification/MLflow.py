...
with mlflow.start_run() as run :
   ...
   #---------- Log Matrics --------------
   mlflow.log_metrics({
   "Model_Score": round(Bagging_model_score, 3),
   ...
   predicted_example = Bagging_model.predict(input_example)
   ...
   mlflow.log_figure(conf_matrix_fig, "plots/Bagging Confusion Matrix.png")
   plt.close()
   
   #----------Log Model -----------
   mlflow.sklearn.log_model(
      sk_model = Bagging_model,
      artifact_path = "model",
      signature = signature,
   ...
   #----------Add Model PKL File ----------- 
   mlflow.log_artifact("Bagging.pkl", artifact_path="pkl_model")
